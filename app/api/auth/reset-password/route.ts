...
export async function POST(req: Request) {
  ...
  try {
    ...
    const transactionResult = await prisma.$transaction(async (tx) => {
      ...
      // Setting a password IS accepting an invite: a still-pending invited
      // user is promoted to Active in the same UPDATE statement (a single
      // statement setting both columns together never risks the DB check
      // constraint — passwordHash IS NOT NULL OR status = 'PENDING_APPROVAL'
      // — seeing an intermediate invalid state). Self-signup users going
      // through forgot-password do NOT get auto-activated this way; they
      // still need explicit Admin approval regardless of their password.
