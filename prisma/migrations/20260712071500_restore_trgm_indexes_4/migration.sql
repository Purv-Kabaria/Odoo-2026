-- Same recurring cause as the three prior restore migrations: pg_trgm GIN
-- indexes aren't expressible in schema.prisma, so every `migrate dev` run
-- drops them as unmanaged drift regardless of which tables the migration
-- actually touches. Recreate them.
